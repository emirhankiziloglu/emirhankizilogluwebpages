					...
					if ( $loop->have_posts() ) {
						while ( $loop->have_posts() ) {
							...
							if ( 'image' === $value['tp_wc_field_type'] ) {
								if( 'yes' === $value['tp_wc_with_url'] ) {
									$output .= '<td> <a href=" ' . get_permalink() . '" target="_blank"> ' . get_the_post_thumbnail() . '</a> </td>';
								} else {
							...
							} elseif ( 'post_url' === $value['tp_wc_field_type'] ) {
									$output .= '<td> <a href=" ' . get_permalink() . '" target="_blank"> <div data-product="' . $product_id . '"> <i aria-hidden="true" class="fas fa-eye"></i> </div> </a></td>';
