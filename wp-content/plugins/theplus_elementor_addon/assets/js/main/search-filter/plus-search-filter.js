					...
					success: function(data){
						if(data){
							...
							seaList.forEach(function(item, index) {
                                ...
                                let getRB = document.querySelectorAll('.elementor-widget-tp-row-background');
								getRB.forEach(function(item,index) {
									console.log(item);
									var trig = item.innerHTML,
										parent = item.closest('.e-con');

									if( parent) {
										parent.insertAdjacentHTML('afterbegin', trig);
									}

									var firstChildDiv = item.querySelector('div'); 
									if (firstChildDiv) {
